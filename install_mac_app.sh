...
on processVideo(inputPath)
    ...
    try
        ...
        display notification "AI rough cut ready" with title "Video Editor"
        set resultDialog to display dialog "Done. AI selected the gameplay and rendered the rough cut." buttons {"Show File", "Open Video"} default button "Open Video" with icon note
...
echo "Each run creates *_rough.mp4 + *_rough.plan.json."
